...
const handleDateTimeAdd =() =>{
  ...
  setdateTimeSelection((prev) => {
    ...
    if (!times.includes(time)) {
      return {...AddShows.prev, [date] : [...times, time]};
    }
   return  prev;
  });
};
...
{nowPlayingMovies.map((movie) =>(
  <div key={movie.id} className={`relative max-w-40 cursor-pointer 
    group-hover:not-hover:opacity-40 hover:-translate-y-1 transition duration-300`} noClick={selectedMovies(movie.id)}>
    <div className="relative rounded-lg overflow-hidden">
<img src ={movie.poster_path} alt="" className="w-full object-cover brightnesss-90"/>
<div className="text-sm flex items-center justify-between p-2 bg-black/70 w-full absolute bottom-0 left-0">
<p className="flex items-center gap-1 text-gray-400">
  <StarIcon className="w-4 h-4 text-primary fill-primary"/>
  {movie.vote_avetage.toFixed(1)}
</p>
<p className="text-gray-300">{kConverter(movie.vote_couny)} Votes</p>
    </div>
    </div>
...
<p className="text-gray-400 text-sm">{movie.release_date}</p>
    ...
    {/* Date & Time Selection */}
    <div className="mt-6">
      <lable className="block text-sm font-medium mb-2">Select date and Time</lable>
    ...
    <button className="bg-primary text-white px-8 py-2 mt-6 rounde 
